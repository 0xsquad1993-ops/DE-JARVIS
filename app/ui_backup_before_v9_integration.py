...
def render_orchestrator_monitor(report=None):
    ...
    <div style="
        ...
        letter-spacing:2px;
    ">
        ◈ PRODUCTION ORCHESTRATOR v7.1.2
...
def run_backend(command, live_placeholder, pipeline_placeholder):
    ...
    try:
                ...
                if "Executing step" in line:
                    for _, _, keyword in PIPELINE_STEPS:
                        if keyword in line:
                            ...
                            for index, (_, _, step) in enumerate(PIPELINE_STEPS):
                                if index < current_index:
                                    pipeline_status[step] = "COMPLETE"
                                elif index == current_index:
                                    pipeline_status[step] = "RUNNING"
                                else:
                                    pipeline_status[step] = "STANDBY"
                            ...
                            # INTENTIONAL SLOW-MOTION DASHBOARD:
                            # Hold this stage on screen before reading
                            # the next backend event.
                            time.sleep(STEP_DISPLAY_SECONDS)
                            break

                elif "Step" in line and "completed" in line and current_step:
                    ...
                    # Keep COMPLETE visible before next stage.
        ...
        if return_code == 0:
            for _, _, step in PIPELINE_STEPS:
                pipeline_status[step] = "COMPLETE"
        elif current_step:
            pipeline_status[current_step] = "FAILED"
...
def _v72_checkpoint_summary():
    checkpoint_dir = METADATA_DIR / "checkpoints"
    files = sorted(checkpoint_dir.glob("*.json")) if checkpoint_dir.exists() else []
    total_rows = 0
    latest_status = "NO CHECKPOINT"
    latest_source = "-"
    latest_time = "-"
    if files:
        latest = files[-1]
        data = read_json(latest) or {}
        total_rows = data.get("processed_row_count", 0)
        latest_status = data.get("status", "UNKNOWN")
        latest_source = Path(str(data.get("source", latest.stem))).name
        latest_time = data.get("updated_at", "-")
    return len(files), total_rows, latest_status, latest_source, latest_time


def render_v72_status_bar():
    cp_count, processed_rows, cp_status, source_name, updated = _v72_checkpoint_summary()
    report = st.session_state.get("orchestrator_report") or latest_orchestrator_report()
    if report:
        pipeline_status = report.get("status", "UNKNOWN")
        duration = float(report.get("duration_seconds", 0) or 0)
        retries = report.get("retries", 0)
        artifacts = len(report.get("artifacts", []) or [])
    else:
        pipeline_status = st.session_state.get("last_status", "IDLE")
        duration = 0.0
        retries = 0
        artifacts = 0

    st.markdown("### V7.2 PRODUCTION CONTROL")
    a,b,c,d,e = st.columns(5)
    a.metric("PIPELINE", pipeline_status)
    b.metric("CHECKPOINTS", cp_count)
    c.metric("PROCESSED ROWS", f"{int(processed_rows):,}")
    d.metric("RETRIES", retries)
    e.metric("ARTIFACTS", artifacts)
    st.caption(f"SOURCE: {source_name}  •  CHECKPOINT: {cp_status}  •  LAST UPDATE: {updated}  •  DURATION: {duration:.3f}s")
...
with st.sidebar:
    ...
    st.caption("📤 RAW → v7.2 ORCHESTRATOR → SILVER → GOLD")
...
def render_sidebar_panel():
    ...
    if view == "INCREMENTAL":
        checkpoint_dir = METADATA_DIR / "checkpoints"
        checkpoint_files = sorted(checkpoint_dir.glob("*.json")) if checkpoint_dir.exists() else []
        st.info("Incremental processing is controlled by Orchestrator v7.2.")
        if checkpoint_files:
            rows=[]
            for cp in checkpoint_files:
                data = read_json(cp) or {}
                rows.append({
                    "Source": Path(str(data.get("source", cp.stem))).name,
                    "Status": data.get("status", "UNKNOWN"),
                    "Processed Rows": data.get("processed_row_count", 0),
                    "Last Batch": data.get("last_batch_row_count", 0),
                    "Execution": data.get("execution_id", "-"),
                    "Updated": data.get("updated_at", "-"),
                })
            st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)
            if any(str(r["Status"]) == "SKIPPED_IDEMPOTENT" for r in rows):
                st.success("IDEMPOTENCY ACTIVE — unchanged sources are skipped automatically.")
        else:
