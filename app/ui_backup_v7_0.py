...
with st.sidebar:
    ...
    st.markdown("### CORE")
    st.write("🧠 Command Agent")
    st.write("⚙ Workflow Planner")
    st.write("◈ Orchestrator")
    st.write("▣ Artifact Manager")

    st.divider()

    st.markdown("### DATA LAYERS")
    st.write(f"RAW       `{count_files(RAW_DIR)}`")
    st.write(f"BRONZE    `{count_files(BRONZE_DIR)}`")
    st.write(f"SILVER    `{count_files(SILVER_DIR)}`")
    st.write(f"GOLD      `{count_files(GOLD_DIR)}`")
    st.write(f"METADATA  `{len(metadata_files())}`")
...
def render_pipeline(statuses=None, placeholder=None):
    ...
    with target.container():
        ...
        for col, (icon, name, keyword) in zip(cols, PIPELINE_STEPS):
            ...
            with col:
                explanations = {
                    "READ_CSV": "Read source CSV",
                    "SCHEMA_ANALYSIS": "Detect columns + types",
                    "SEMANTIC_ANALYSIS": "Understand column roles",
                    "DATA_QUALITY": "Check nulls + duplicates",
                    "VALIDATE_DATA": "Validate data integrity",
                    "TRANSFORMATION": "Clean + standardize",
                    "SAVE_SILVER": "Write clean Silver data",
                    "GOLD_ANALYTICS": "Build Gold analytics",
                }
...
st.html(
    ...
    <div class="footer">
        ...
        &nbsp;•&nbsp;
        COMMAND → PLAN → EXECUTE → VALIDATE → SILVER → GOLD
    </div>
    """
)
